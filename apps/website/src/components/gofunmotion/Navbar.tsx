...
export function Logo() {
  return <Link aria-label="GoFunMotion home" className="inline-flex min-h-11 shrink-0 items-center gap-2 text-base font-bold" href="/">
    <img alt="" aria-hidden="true" className="hidden size-8 min-[380px]:block" height={32} src="/brand/gofunmotion-mark.svg" width={32} />
    <span>Go<span className="text-[var(--accent-lime)]">Fun</span>Motion</span>
  </Link>;
}
...
export function Navbar() {
  ...
  >{link.label}</Link>;
  return <header className="sticky top-0 z-40 h-[var(--app-header-height)] border-b border-[var(--border-subtle)] bg-[var(--panel-strong)] backdrop-blur-xl">
    <div className="mx-auto flex h-full max-w-7xl items-center justify-between gap-2 px-4 md:px-8">
      ...
      <div className="hidden items-center gap-2 xl:flex"><ThemeToggle /><AccountNav /></div>
